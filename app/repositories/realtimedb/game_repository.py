...
class GameRepository(RealTimeBaseRepository):
    ...
    def set_player_color(self, room_id: str, player_stt: str, color: str) -> None:
        """Set color for a specific player in Round 4"""
        logger.info(f"set_player_color {color}")
        logger.info(f"set_player_color {room_id}")
        logger.info(f"set_player_color {player_stt}")
        self.set_to_path(f"{room_id}/player_colors/{player_stt}", color)

    def remove_player_color(self, room_id: str, player_stt: str) -> None:
        """Remove color for a specific player in Round 4"""
        self.delete_path(f"{room_id}/player_colors/{player_stt}")

    def get_player_color(self, room_id: str, player_stt: str) -> str:
        """Get color for a specific player"""
        return self.read_from_path(f"{room_id}/player_colors/{player_stt}")

    def get_all_player_colors(self, room_id: str) -> Dict[str, str]:
        """Get all player colors"""
        return self.read_from_path(f"{room_id}/player_colors") or {}
    ...
    def set_used_packets(self, room_id: str, used_packets: List[str]) -> None:
        self.set_to_path(f"{room_id}/usedPackets", used_packets)
    ...
    def set_selected_row(self, room_id: str, selected_row_number: str, selected_row_index: int, selected_col_index: int, is_row: bool,word_length: int):
        self.set_to_path(f"{room_id}/select", {"selected_row_number": selected_row_number, "is_row": is_row, "word_length": word_length
                                                ,"selected_row_index": selected_row_index, "selected_col_index": selected_col_index})

    def set_incorrect_row(self, room_id: str, selected_row_number: str, selected_row_index: int, selected_col_index: int, is_row:bool, word_length: int):
        self.set_to_path(f"{room_id}/incorrect", {"selected_row_number": selected_row_number, "is_row": is_row, "word_length": word_length
                                                  ,"selected_row_index": selected_row_index, "selected_col_index": selected_col_index})

    def set_correct_row(self, room_id: str, selected_row_number: str, selected_row_index: int, selected_col_index: int, correct_answer: str, marked_character_index: str, is_row:bool):
        logger.info(f"marked_character_index {json.loads(marked_character_index)}")
    ...
    def send_obstacle(self, room_id:str, grid: Grid):
        ...
        logger.info(f"obstacle {grid.obstacle}")
        self.set_to_path(f"{room_id}/obstacle",{
    ...
    def get_player_answer(self, room_id: str, uid: str) -> Dict[str, Any]:
        logger.info(f"Getting player answer for room {room_id}, uid {uid}")
        return self.read_from_path(f"{room_id}/player_answer/{uid}")
    
    def get_all_player_answer(self, room_id: str) -> List[Dict[str, Any]]:
        return self.read_from_path(f"{room_id}/player_answer")
    
    def get_player_answer_list(self, room_id: str) -> List[Dict[str, Any]]:
        return self.read_from_path(f"{room_id}/answerLists")
    ...
    def reset_star(self,room_id: str):
        self.delete_path(f"{room_id}/star")
